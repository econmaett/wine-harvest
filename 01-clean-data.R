...
write_csv(x = wineharvest_wide, file = "clean_wineharvest_wide.csv")
...
write_csv(x = locations, file = "clean_locations.csv")
...
write_csv(x = wineharvest_long, file = "clean_wineharvest_long.csv")
...
p1 <- ggplot(data = wineharvest_wide, mapping = aes(x = year, y = bur)) +
  ...
  labs(
    title = "Annual wine harvest in Burgundy, France",
...
ggsave(filename = "fig_burgundy.png", plot = p1, path = ".", width = 24, height = 12, units = "cm", bg = "white")

variable_selection <- c("bur", "jur", "srv", "swi")

p2 <- ggplot(data = wineharvest_long |> filter(abbreviation %in% variable_selection), mapping = aes(x = year, y = value)) +
  # geom_hline(yintercept = 0, linewidth = 0.5, colour = "darkgray") +
  geom_line(linewidth = 0.5, colour = "gray", alpha = 0.25) +
  geom_point(size = 1) +
  geom_smooth(method = "loess", formula = "y ~ x", se = TRUE, colour = "#800020", fill = "#800020", span = 0.1, alpha = 0.2) +
...
ggsave(filename = "fig_wineharvest.png", plot = p2, path = ".", width = 20, height = 24, units = "cm", bg = "white")
